...
export default function ChatDialogueScene({ scene, onComplete, protagonistImage }: Props) {
      ...
      {scene.location && (
        <>
          <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
            <SceneEnvironment location={scene.location} opacity={0.75} showCalendar={false} />
          </div>
          {/* Darken overlay — lighter at top so background reads */}
          <div style={{
            position:   "absolute",
            inset:      0,
            zIndex:     1,
            background: "linear-gradient(180deg, rgba(8,6,15,0.38) 0%, rgba(6,4,14,0.52) 50%, rgba(6,4,14,0.95) 100%)",
          }} />
        </>
      )}
